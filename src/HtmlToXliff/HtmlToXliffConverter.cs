...
namespace HtmlToXliff
{
	...
	public class HtmlToXliffConverter
	{
		...
		/// <summary>
		/// Users must create one of these with either Load or Parse, which either Loads an
		/// HTML file (and then parses it), or Parses an HTML string to initialize the object.
		/// </summary>
		private HtmlToXliffConverter(string html, string filename)
		{
		...
		/// </remarks>
		private static void FixHtmlAgilityParserBug()
		{
		...
		/// <summary>
		/// Create a converter object and initialize it by loading and parsing the specified HTML.
		/// </summary>
		public static HtmlToXliffConverter Load(string filename)
		{
			FixHtmlAgilityParserBug();
			return new HtmlToXliffConverter(File.ReadAllText(filename, Encoding.UTF8), filename);
		}

		/// <summary>
		/// Create a converter object and initialize it by by parsing the specified HTML string. The optional
		/// filename argument will be used as the "original" attribute of the "file" element in the XLIFF.
		/// </summary>
		public static HtmlToXliffConverter Parse(string html, string filename = "test.html")
		{
			FixHtmlAgilityParserBug();
			return new HtmlToXliffConverter(html, filename);
		}
